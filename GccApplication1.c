...
#include "C:\Users\student\Desktop\header files (1)\io.c"
#include "C:\Users\student\Desktop\header files (1)\io.h"
...
enum SL_States{ SL_Start, SL_Play, SL_Press, Complete } SL_state;
unsigned char debug;
unsigned char first;
unsigned char sec;
unsigned char third;
...
int SM_SlotMachine ( )
{
	unsigned char specChar[4] = {0xA5, 0xA7, 0xA3, 0x3C};
	unsigned char specChar1[4] = { 0xA7, 0xA5, 0x3C, 0xA3};
	unsigned char specChar2[4] = { 0xA3, 0x3C, 0xA7, 0xA5};
	
	static unsigned char index;
	static unsigned char presses;
	//static unsigned cha
	switch( SL_state )
	{
		case SL_Start:
		...
		case SL_Play:			
			...
			if( presses == 0)
			{
				PORTB = 0x01;
				LCD_Cursor( 6 );
				LCD_WriteData( specChar[index]);
				
				LCD_Cursor( 8 );
				LCD_WriteData( specChar1[index]);
				
				LCD_Cursor( 10 );
				LCD_WriteData( specChar2[index]);
		...
		case Complete:
			if( specChar[first] == specChar1[sec] )
			{
				if(specChar1[sec] == specChar2[third] )
					LCD_DisplayString(1, "POWER UP i" );
				else
					LCD_DisplayString(1, "Almost");
			}
			else
				LCD_DisplayString( 1, "NADA");
			break;
	}
